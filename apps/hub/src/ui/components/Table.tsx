...
const HeaderCell = React.forwardRef<HTMLDivElement, HeaderCellProps>(function HeaderCell(
  ...
  return (
    <th {...otherProps}>
      <div
        className={SubframeUtils.twClassNames('flex h-8 w-full items-center gap-1 px-3', className)}
        ref={ref}
      >
        {children ? (
          <span className="whitespace-nowrap text-caption-small font-caption-small text-subtext-color">
            {children}
          </span>
        ) : null}
        {icon ? (
