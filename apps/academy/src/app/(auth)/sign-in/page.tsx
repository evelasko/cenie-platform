...
export default function AcademySignInPage() {
  return (
    <Suspense fallback={<div className="text-center text-slate-300 p-8">Loading...</div>}>
      <AcademySignInContent />
    </Suspense>
  )
}

function AcademySignInContent() {
  // Form state
  ...
  const checkAccessAndCreateSession = async (user: { uid: string; getIdToken: () => Promise<string> }) => {
    try {
      ...
      const dashboardPath =
        ACADEMY_CONFIG.roles[accessData.role as keyof typeof ACADEMY_CONFIG.roles]
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl p-8">
      ...
      <p className="text-gray-600 text-center mb-6">Sign in to continue your learning journey</p>
