...
-- Migration: 20250127_books_management_fixed
...
-- =====================================================
-- USER APP ACCESS TABLE (if not exists)
-- =====================================================
-- Ensure the user_app_access table exists for role-based permissions
...
-- Authenticated users can insert books (role check happens in application layer)
CREATE POLICY "Authenticated users can insert books"
  ON books FOR INSERT
  TO authenticated
  WITH CHECK (true);

-- Authenticated users can update books (role check happens in application layer)
CREATE POLICY "Authenticated users can update books"
  ON books FOR UPDATE
  TO authenticated
  USING (true);

-- Authenticated users can delete books (role check happens in application layer)
CREATE POLICY "Authenticated users can delete books"
  ON books FOR DELETE
  TO authenticated
  USING (true);

-- Drop existing book tags policies
DROP POLICY IF EXISTS "Anyone can view book tags" ON book_tags;
DROP POLICY IF EXISTS "Authenticated users can manage book tags" ON book_tags;
...
CREATE POLICY "Authenticated users can manage book tags"
  ON book_tags FOR ALL
  TO authenticated
  USING (true)
  WITH CHECK (true);

-- Drop existing book reviews policies
DROP POLICY IF EXISTS "Anyone can view book reviews" ON book_reviews;
DROP POLICY IF EXISTS "Users can create reviews" ON book_reviews;
DROP POLICY IF EXISTS "Users can update their own reviews" ON book_reviews;
DROP POLICY IF EXISTS "Users can delete their own reviews" ON book_reviews;
...
-- Function to grant editorial access to a user (run this after migration)
-- Usage: SELECT grant_editorial_access('user-uuid-here', 'admin');
...
COMMENT ON FUNCTION grant_editorial_access IS 'Grant editorial access to a user. Role can be "admin" or "editor".';
