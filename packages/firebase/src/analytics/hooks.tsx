...
import { type AnalyticsEventParams } from '../analytics-utils'
...
export function useAnalytics() {
  const { logEvent, logPageView, logUserAction, logError, isInitialized, appName } =
    useAnalyticsContext()
 ...
 */
export function usePageTracking(enabled = true, customTitle?: (pathname: string) => string) {
...
export function useUserTracking() {
  ...
  const trackClick = useCallback(
    (elementId: string, elementType = 'button', additionalParams?: AnalyticsEventParams) => {
  ...
  const trackFormSubmission = useCallback(
    (formName: string, successful = true, additionalParams?: AnalyticsEventParams) => {
      if (!isInitialized) return

      logUserAction(successful ? 'form_submit_success' : 'form_submit_error', 'form', formName)
  ...
  const trackSearch = useCallback(
    (searchTerm: string, searchCategory?: string, resultCount?: number) => {
      if (!isInitialized) return
...
export function useErrorTracking() {
  ...
  const trackValidationError = useCallback(
    (formField: string, validationType: string, errorMessage: string) => {
      if (!isInitialized) return
...
export function usePerformanceTracking() {
  ...
  const trackTiming = useCallback(
    (name: string, value: number, category = 'performance') => {
      if (!isInitialized) return
